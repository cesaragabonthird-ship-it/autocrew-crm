...
/**
 * AutoCrew CRM — Convex Schema
 * Replaces all Google Sheets tabs.
...
export default defineSchema({
  ...
  tenants: defineTable({
    uid:             v.string(),   // Firebase UID of owner
    shopName:        v.string(),
    ownerName:       v.string(),
    email:           v.string(),
    phone:           v.string(),
    plan:            v.string(),   // starter | growth | pro
    status:          v.string(),   // trial | active | grace | suspended | cancelled
  ...
  users: defineTable({
    ...
    role:         v.string(),   // super_admin | branch_manager | sales_staff | installer | staff
  ...
  purchaseOrders: defineTable({
    ...
    supplier:       v.string(),
    branch:         v.string(),
    status:         v.string(),   // draft | ordered | partial | received | cancelled
    orderDate:      v.string(),
  ...
  deliveries: defineTable({
    ...
    supplier:       v.string(),
    branch:         v.string(),
    status:         v.string(),   // pending | received | partial | issue
  ...
  quotes: defineTable({
    ...
    status:      v.string(),   // draft | sent | accepted | declined | expired
  ...
  .index("by_assignedUid",      ["tenantId", "assignedUid"])
  ...
  invoices: defineTable({
    ...
    vehicle:       v.optional(v.string()),
    branch:        v.string(),
    status:        v.string(),   // draft | sent | paid | partial | overdue
    issueDate:     v.string(),
    dueDate:       v.string(),
  ...
  payments: defineTable({
    ...
    branch:        v.string(),
    amount:        v.number(),
    method:        v.string(),   // Cash | GCash | Maya | Bank Transfer | etc
  ...
  payrollEmployees: defineTable({
    ...
    branch:             v.string(),
    department:         v.string(),
    employmentType:     v.string(),   // Regular | Probationary | Contractual | Part-time
    payType:            v.string(),   // monthly | daily | hourly
  ...
  .index("by_tenantId",        ["tenantId"])
  .index("by_tenantId_branch", ["tenantId", "branch"])
  .index("by_employeeId",      ["tenantId", "employeeId"]),
  ...
  attendance: defineTable({
    ...
    date:            v.string(),
    status:          v.string(),   // present | absent | half | late | leave_sl | leave_vl | holiday | restday
  ...
  .index("by_tenantId", ["tenantId"])
  .index("by_period",   ["tenantId", "periodId"]),
  ...
  .index("by_payrollEmployeeId", ["payrollEmployeeId"])
  .index("by_tenantId_employee", ["tenantId", "employeeId"]),

  // ── Payroll Settings ──────────────────────────────────────
  payrollSettings: defineTable({
    tenantId:    v.id("tenants"),
    settingsJson:v.string(),   // full JSON of DEFAULT_PAYROLL_SETTINGS
  })
  .index("by_tenantId", ["tenantId"]),
